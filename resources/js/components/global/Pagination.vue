<template>
    <ul v-if="totalPage > 0" class="pagination">
        <li :class="{ disabled: internalCurrentPage === 1 }">
            <a class="previous" href="javascript:;" @click="changePage(internalCurrentPage - 1)"><</a>
        </li>
        ...
        <li :class="{ disabled: internalCurrentPage === totalPage }" >
            <a class="next" href="javascript:;" @click="changePage(internalCurrentPage + 1)">></a>
        </li>
    </ul>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            // 总页数
            totalPage() {
                return Math.ceil(this.total / parseInt(this.pageSize))
            }
        },
        watch: {
            currentPage: {
